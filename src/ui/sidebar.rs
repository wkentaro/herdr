...
pub(crate) fn normalized_workspace_scroll(app: &AppState, area: Rect, requested: usize) -> usize {
    let ws_area = workspace_list_rect(area, app.sidebar_section_split, app.sidebar_agents.visible);
...
pub(crate) fn workspace_list_rect(area: Rect, split_ratio: f32, agent_panel_visible: bool) -> Rect {
    let (ws_area, _) = expanded_sidebar_sections(area, split_ratio, agent_panel_visible);
    ws_area
}
...
pub(crate) fn collapsed_sidebar_sections(
    ...
    if !agent_panel_visible {
        // Keep the bottom row available for reopening the expanded sidebar.
        return (
            Rect::new(
                content.x,
                content.y,
...
pub(super) fn render_sidebar_collapsed(app: &AppState, frame: &mut Frame, area: Rect) {
    ...
    let (ws_area, divider_y, detail_area) =
        collapsed_sidebar_sections(area, app.sidebar_agents.visible);
    if ws_area == Rect::default() {
...
pub(super) fn render_sidebar(
    ...
    let (ws_area, detail_area) =
        expanded_sidebar_sections(area, app.sidebar_section_split, app.sidebar_agents.visible);
        ...
        for (tab_idx, tab) in ws.tabs.iter().enumerate() {
            ...
            let label = truncate_end(
                ...
                card.rect.width.saturating_sub(
                    display_width_u16(prefix)
                        .saturating_add(display_width_u16(connector))
                        .saturating_add(2),
                ) as usize,
            );
            let (tab_state, tab_seen) = tab.aggregate_state(&app.terminals);
            let (tab_icon, tab_icon_style) =
                state_icon(tab_state, tab_seen, app.status_indicators, p);
            ...
            frame.buffer_mut().set_line(
                ...
                &Line::from(vec![
                    ...
                    Span::styled(tab_icon, tab_icon_style),
                    Span::raw(" "),
...
"##,
    ...
    fn selected_active_workspace_resolves_collapsed_background() {
        ...
        let (workspace_area, _, _) = collapsed_sidebar_sections(area, app.sidebar_agents.visible);
        assert_eq!(
...
"##,
    ...
    fn oversized_space_layout_is_clipped_to_the_section_body() {
        ...
        let workspace_area =
            workspace_list_rect(area, app.sidebar_section_split, app.sidebar_agents.visible);
    ...
    fn collapsed_sidebar_numbers_grouped_agents_by_list_position() {
        ...
        let (_, _, detail_area) = collapsed_sidebar_sections(area, app.sidebar_agents.visible);
    ...
    #[test]
    fn hidden_agent_panel_gives_full_sidebar_to_workspaces() {
        let area = Rect::new(0, 0, 20, 12);

        assert_eq!(
            expanded_sidebar_sections(area, 0.5, false),
            (Rect::new(0, 0, 19, 12), Rect::default())
        );
        assert_eq!(
            collapsed_sidebar_sections(area, false),
            (Rect::new(0, 0, 19, 11), None, Rect::default())
        );
        assert_eq!(
    ...
    #[test]
    fn workspace_list_shows_tabs_for_every_workspace() {
        ...
        let buffer = terminal.backend().buffer();
        assert!(row_text(buffer, cards[0].rect.y, area.width).contains("single"));
        assert!(!row_text(buffer, cards[0].rect.y, area.width).contains('●'));
        assert!(row_text(buffer, cards[0].rect.y + 1, area.width).contains("└─ ● notes"));
    ...
    fn space_row_gap_preserves_compact_worktree_children() {
        ...
        let spacious_metrics = workspace_list_scroll_metrics(&app, Rect::new(0, 0, 30, 11));
        ...
        let packed_metrics = workspace_list_scroll_metrics(&app, Rect::new(0, 0, 30, 11));
    ...
    fn packed_workspace_drag_indicator_overlays_an_internal_boundary() {
        ...
        assert_eq!(
            indicator_row,
            app.view.workspace_card_areas[1]
                .rect
                .y
                .saturating_add(app.view.workspace_card_areas[1].rect.height - 1)
        );
