...
use self::dialogs::{
    render_confirm_close_overlay, render_confirm_kill_session_overlay,
...
pub(crate) use self::{
    dialogs::{
        confirm_close_button_rects, confirm_close_popup_rect, confirm_kill_session_button_rects,
        confirm_kill_session_popup_rect, new_linked_worktree_button_rects,
    ...
    sidebar::{
        ...
        workspace_drop_slots, workspace_list_entries, workspace_list_rect,
...
fn compute_view_internal(
    ...
    let (workspace_card_areas, workspace_folder_areas) = if app.sidebar_collapsed {
        (Vec::new(), Vec::new())
    } else {
...
pub fn render_with_runtime_registry(
    ...
    match app.mode {
        ...
        Mode::ConfirmKillSession => render_confirm_kill_session_overlay(app, frame, terminal_area),
        Mode::ContextMenu => {
        ...
        | Mode::RenameTab
        | Mode::RenamePane => render_rename_overlay(app, frame, frame.area()),
