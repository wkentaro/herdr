...
Killing a whole named session is intentionally unbound. To opt in, set `kill_session` to a key such as `prefix+shift+d`; if that key currently closes workspaces, clear `close_workspace` first. Herdr will ask for confirmation, then stop every pane process and permanently delete the session's saved workspace state and pane history.
