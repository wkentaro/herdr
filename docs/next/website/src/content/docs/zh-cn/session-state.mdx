...
停止会话会结束窗格进程,但保留 `session.json` 和已保存的窗格历史,以便以后恢复布局。delete 删除已经停止的命名会话。kill 是两者组合成的破坏性操作:Herdr 会优雅停止命名会话,等待最后一次保存和套接字关闭,然后永久删除保存的工作区状态和窗格历史。

默认会话可以 stop,但不能 delete 或 kill。`keys.kill_session` 动作默认未设置,并且在请求 kill 前始终显示确认界面。
