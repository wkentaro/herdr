...
Right-click a workspace and choose **Hide** to move it into the collapsed **Hidden** tree at the bottom of the sidebar. Its terminals and agents keep running, but it and its agents no longer appear in normal navigation. Expand Hidden and click a workspace to restore and focus it; each entry includes its machine label.

Hiding the active workspace switches to another visible workspace. Hiding the last visible workspace opens a replacement using the same default directory behavior as closing the last workspace. Linked worktrees are hidden individually. Hidden choices are saved on this client across restarts. Notifications remain enabled, and clicking one restores its workspace. Keyboard users can open a workspace's menu with `shift+F10` in workspace navigation and restore hidden workspaces through the navigator.
