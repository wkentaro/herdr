...
A workspace is a terminal work context. A project can use several workspaces for separate tasks, investigations, or terminal layouts.
...
Workspace folders organize related workspaces without changing what a workspace owns. Create and rename folders from the sidebar menu, then drag workspaces into, within, or out of them. Folders are one level deep, stay in the order you choose, and never infer membership from Git repositories or worktrees.
