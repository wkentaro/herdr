...
| セッション | `session.snapshot`、`session.kill` |
...
`session.snapshot` は、独自のローカルランタイムキャッシュを持つクライアント向けに、一度限りのブートストラップスナップショットを返します。レスポンスには、セッション名、バージョン/プロトコルメタデータ、フォーカス中のワークスペース/タブ/ペイン id、ワークスペース、タブ、ペイン、タブレイアウト、エージェントの各レコードが含まれます。これは購読ではありません。取得後はリソースイベントを購読し、そのイベントでローカルキャッシュを更新してください。再接続後やキャッシュが古い可能性がある場合は、`session.snapshot` を再度呼び出します。ワークスペースレコードには、関連付けられた worktree の出自情報も含まれます。リポジトリ全体の worktree 検出には引き続き `worktree.list` を使います。

CLI の `herdr api snapshot` は、クライアントやエージェントが簡単にブートストラップできるよう、現在の `session.snapshot` レスポンスを JSON で出力します。

`session.kill` は空の params を受け付け、名前付きセッションでのみ利用できます。成功レスポンスは、デタッチされた kill ヘルパーが受理されたことを意味します。その後ヘルパーがサーバーを正常終了し、そのセッションの保存済みワークスペース状態とペイン履歴を完全に削除します。デフォルトセッションは `default_session_protected` を返します。
