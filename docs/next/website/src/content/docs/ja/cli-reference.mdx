...
`stop` はペインのプロセスを終了しますが、後で復元できるよう保存済み状態を残します。`delete` は停止済みの名前付きセッションを削除します。`kill` はこの 2 つを順番に実行し、保存済みのワークスペース状態とペイン履歴を完全に削除します。デフォルトセッションは `default` という名前で停止できますが、削除も kill もできません。
