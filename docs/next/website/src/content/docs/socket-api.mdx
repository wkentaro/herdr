...
| Session | `session.snapshot`, `session.kill` |
| Workspace | `workspace.create`, `workspace.list`, `workspace.get`, `workspace.focus`, `workspace.rename`, `workspace.move`, `workspace.move_block`, `workspace.folder.create`, `workspace.folder.rename`, `workspace.folder.delete`, `workspace.folder.move`, `workspace.report_metadata`, `workspace.close` |
...
`workspace.list` and `session.snapshot` include `workspace_layout`, an ordered list of root workspaces and one-level folders. A folder has a stable `folder_id`, a name, and ordered `workspace_ids`. `workspace.create` accepts an optional `folder_id`; `workspace.move` accepts an optional `folder_id` plus the insertion index within that destination. Folder create, rename, delete, and root-level move operations emit `workspace.layout_updated`. Deleting a folder keeps its workspaces at the same root position.

`session.snapshot` returns a one-time bootstrap snapshot for clients that keep
their own local runtime cache. The response includes the session name,
version/protocol metadata, focused workspace/tab/pane ids, workspace records,
tab records, pane records, tab layout snapshots, and agent records. It is not a
subscription; after reading it, subscribe to resource events and update the
local cache from those events.
...
`session.kill` accepts empty params and is only available in a named session. A successful response means the detached kill helper was accepted; the helper then gracefully stops the server and permanently deletes that session's saved workspace state and pane history. The default session returns `default_session_protected`.
...
Workspace event subscriptions include `workspace.created`, `workspace.updated`, `workspace.metadata_updated`, `workspace.renamed`, `workspace.moved`, `workspace.reordered`, `workspace.layout_updated`, `workspace.closed`, and `workspace.focused`. `workspace.metadata_updated` reports token changes and TTL expiry without invoking plugin event hooks. Other workspace events describe Herdr UI/runtime lifecycle. `workspace.created` includes optional `workspace.worktree` provenance. `workspace.moved` includes the moved `workspace_id`, requested `insert_index`, and updated ordered `workspaces` list. `workspace.reordered` includes the atomically moved `workspace_ids`, optional `before_workspace_id`, and authoritative ordered `workspaces` list. `workspace.layout_updated` contains the authoritative workspace/folder layout. `workspace.closed` includes a final `workspace` snapshot when Herdr can still identify it before removal.
