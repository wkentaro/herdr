...
Set `pane_dim = true` to dim inactive split panes so the focused pane stands out, similar to WezTerm. Unlike the default dimming that only applies while navigating, this keeps inactive panes dimmed even while a pane is active in terminal mode. Combine it with `pane_borders = false` and `pane_gaps = false` for a minimal, borderless layout where focus is shown by dimming alone.
