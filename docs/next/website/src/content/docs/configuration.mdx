...
Workspaces with multiple tabs show those tabs as clickable child rows in the expanded Spaces list. Like the Go to picker, single-tab workspaces omit the redundant tab level.
...
Set `visible = false` under `[ui.sidebar.agents]` to hide the Agent section in expanded and compact desktop sidebars. The Spaces section then uses the full sidebar height.
