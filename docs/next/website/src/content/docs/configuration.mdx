...
Every workspace shows its tabs as clickable child rows in the expanded Spaces list. Each row carries that tab's agent-status indicator, so attention can be located before switching tabs.
