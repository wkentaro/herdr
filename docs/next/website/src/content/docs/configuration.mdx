...
Worktree actions are available from Git workspace rows. `New worktree` creates a checkout. It checks out an existing local branch when the entered branch exists; otherwise, it creates the branch. `Open worktree...` lists existing Git worktree checkouts for that repo. Choosing an already-open checkout focuses it, while choosing a closed checkout opens it as another workspace. Herdr does not group workspaces automatically.

Use `New folder` in the sidebar menu to organize workspaces explicitly. Folder and workspace rows can be reordered by dragging; drop a workspace on a folder to append it, between folder children to place it, or at a root gap to remove it from a folder. Right-click a folder to create a workspace inside it, rename it, or delete the folder while keeping its workspaces. Closing any workspace closes only that workspace.

The mobile switcher shows the same hierarchy and lets you expand or collapse folders, but folder editing remains on the desktop sidebar.

To delete a worktree checkout, use `Delete worktree checkout...` on its workspace. Herdr runs `git worktree remove`, first asking Git to remove safely. If Git refuses because the checkout has modified or untracked files, Herdr asks again before running the forced remove. Branches are not deleted.
