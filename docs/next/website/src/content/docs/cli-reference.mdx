...
herdr session kill <name> [--json]
herdr session delete <name> [--json]
```

`stop` ends pane processes but keeps saved state for a later restore. `delete` removes an already stopped named session. `kill` performs both operations in order and permanently removes its saved workspace state and pane history. The default session can be stopped with the name `default`, but it cannot be deleted or killed.
...
A workspace is a terminal work context. Creating one also creates its first tab and root pane. The JSON response exposes their IDs as `.result.workspace.workspace_id`, `.result.tab.tab_id`, and `.result.root_pane.pane_id`.
...
Worktrees are normal Herdr workspaces with Git checkout provenance. `worktree create` creates a Git worktree checkout and opens it as a workspace. If `--branch` names an existing local branch, Herdr checks it out; otherwise it creates the branch from `--base` or `HEAD`. Without `--path`, Herdr creates the checkout under `<worktrees.directory>/<repo>/<branch-slug>`. Organize it with other workspaces manually from the sidebar when useful.
...
A tab is another terminal layout inside a workspace. Without `--workspace`, `tab create` uses the active workspace and fails if none exists. Its JSON response exposes `.result.tab.tab_id` and `.result.root_pane.pane_id`. Closing a workspace's last tab also closes that workspace, matching the TUI close-tab action.
